...
class BigBirdAttention(nn.Module):
    def __init__(self, config, block_size=16, num_random_blocks=2):
    ...
    def forward(
        ...
        # 确保序列长度是块大小的整数倍，这是BigBird要求的条件
        if from_seq_length % from_block_size != 0:
            raise ValueError(f"Query sided sequence length must be multiple of block size, where seq_len = {from_seq_length} and block size = {from_block_size}")

        if to_seq_length % to_block_size != 0:
            raise ValueError(f"Key/Value sided sequence length must be multiple of block size, where seq_len = {to_seq_length} and block size = {to_block_size}")
        ...
        if attention_mask is not None:
            attention_mask = self.transform_mask(attention_mask)
        else:   
            attention_mask = torch.ones(batch_size, seqlen).to(device)
        from_blocked_mask, band_mask, from_mask, to_mask = self.create_masks_for_block_sparse_attn(attention_mask, self.block_size)
        to_blocked_mask = from_blocked_mask
    ...
    def transform_mask(self, attention_mask):
        attention_mask = attention_mask.squeeze(1)[:, 0, :].squeeze(1)
        return (attention_mask > -1e4).float()
