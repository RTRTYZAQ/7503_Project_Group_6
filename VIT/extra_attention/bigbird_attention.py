...
class BigBird_Attention(nn.Module):
    def __init__(self, config, vis, block_size=8, num_random_blocks=2):
        super(BigBird_Attention, self).__init__()
        self.vis = vis
    ...
    def forward(self, hidden_states):
        ...
        batch_size, orig_seqlen, hidden_dim = hidden_states.size()

        # 检查是否需要填充
        pad_len = 0
        if orig_seqlen % self.block_size != 0:
            pad_len = self.block_size - (orig_seqlen % self.block_size)
            padding = torch.zeros(batch_size, pad_len, hidden_dim, device=device)
            hidden_states = torch.cat([hidden_states, padding], dim=1)
        
        seqlen = pad_len + orig_seqlen
        # 对于自注意力，查询和键值的序列长度相同
